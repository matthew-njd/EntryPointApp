export const ES = {
  nav: {
    logout: 'Cerrar sesión',
    language: 'English',
  },
  footer: {
    support: 'Ayuda:',
    copyright:
      'Copyright © 2026 - Todos los derechos reservados por The Postcard Factory',
  },
  common: {
    back: 'Atrás',
    cancel: 'Cancelar',
    save: 'Guardar cambios',
    saving: 'Guardando...',
    view: 'Ver',
    edit: 'Editar',
    support: 'Soporte',
    appTitle: 'Aplicación de registro de horas de PCF',
    hrs: 'hrs',
    mi: 'mi',
  },
  auth: {
    login: {
      legend: 'Iniciar sesión',
      email: 'Correo electrónico*',
      emailPlaceholder: 'Correo electrónico',
      password: 'Contraseña*',
      passwordPlaceholder: 'Contraseña',
      forgotPassword: '¿Olvidó su contraseña?',
      submit: 'Iniciar sesión',
      loggingIn: 'Iniciando sesión...',
      createAccount: 'Crear cuenta',
      register: 'Registrarse',
    },
    register: {
      legend: 'Registrarse',
      firstName: 'Nombre*',
      firstNamePlaceholder: 'Nombre',
      lastName: 'Apellido*',
      lastNamePlaceholder: 'Apellido',
      email: 'Correo electrónico*',
      emailPlaceholder: 'Correo electrónico',
      password: 'Contraseña*',
      passwordPlaceholder: 'Contraseña',
      confirmPassword: 'Confirmar contraseña*',
      confirmPasswordPlaceholder: 'Confirmar contraseña',
      confirmEmail: 'Confirmar correo electrónico*',
      confirmEmailPlaceholder: 'Confirmar correo electrónico',
      submit: 'Registrarse',
      registering: 'Registrando...',
      back: 'Atrás',
    },
    forgotPassword: {
      legend: 'Restablecer contraseña',
      email: 'Correo electrónico*',
      emailPlaceholder: 'Correo electrónico',
      submit: 'Enviar enlace de restablecimiento',
      sending: 'Enviando...',
    },
    resetPassword: {
      legend: 'Crear nueva contraseña',
      invalidLink:
        'Enlace de restablecimiento inválido. Por favor solicite un nuevo restablecimiento de contraseña.',
      newPassword: 'Nueva contraseña*',
      newPasswordPlaceholder: 'Nueva contraseña',
      confirmPassword: 'Confirmar contraseña*',
      confirmPasswordPlaceholder: 'Confirmar contraseña',
      submit: 'Restablecer contraseña',
      resetting: 'Restableciendo...',
    },
  },
  errors: {
    emailRequired: 'El correo electrónico es requerido',
    emailInvalid: 'Formato de correo electrónico inválido',
    passwordRequired: 'La contraseña es requerida',
    passwordMinLength: 'La contraseña debe tener al menos 8 caracteres.',
    passwordStrength:
      'La contraseña debe contener mayúsculas, minúsculas, números y caracteres especiales.',
    firstNameRequired: 'El nombre es requerido',
    lastNameRequired: 'El apellido es requerido',
    confirmPasswordRequired: 'Por favor confirme su contraseña',
    passwordMismatch: 'Las contraseñas no coinciden',
    confirmEmailRequired: 'Por favor confirme su correo electrónico',
    emailMismatch: 'Las direcciones de correo electrónico no coinciden',
    roleRequired: 'El rol es requerido',
    startDateRequired: 'La fecha de inicio es requerida',
    denyReasonRequired: 'Se requiere un motivo al denegar una hoja de tiempo',
    maxLength500: 'Máximo 500 caracteres',
    timeInRequired: 'La hora de entrada es requerida',
    timeOutRequired: 'La hora de salida es requerida',
    timeOutAfterTimeIn:
      'La hora de salida debe ser posterior a la hora de entrada',
    fixDayErrors:
      'Por favor, corrija los errores en los días resaltados arriba antes de continuar.',
    mustBePositive: 'El valor debe ser 0 o mayor',
    mileageMax: 'El millaje no puede exceder 500 mi',
    chargeMax: 'El monto no puede exceder $999.99',
    commentMaxLength: 'El comentario no puede exceder 500 caracteres',
    timesheetOverlap: 'Ya existe una hoja de tiempo para esta semana',
    weekMustStartMonday: 'Las hojas de tiempo deben comenzar un lunes',
    weekInFuture: 'No se puede crear una hoja de tiempo para una semana futura',
    weekTooFarInPast:
      'No se puede crear una hoja de tiempo con más de 8 semanas de antigüedad',
    hourlyRateRequired: 'Se requiere una tarifa por hora válida',
    mileageRateRequired: 'Se requiere una tarifa de millaje válida',
    effectiveDateRequired: 'La fecha efectiva es requerida',
  },
  dashboard: {
    newTimesheet: 'Nuevo registro de horas',
    hello: 'Hola,',
    totalApproved: 'Total aprobadas',
    totalPending: 'Total pendientes',
    totalDenied: 'Total denegadas',
    totalDrafts: 'Total borradores',
    tableWeek: 'Semana',
    tableTotalHours: 'Total de horas',
    tableTotalCharges: 'Total de cargos',
    tableStatus: 'Estado',
    noTimesheets: 'No se encontraron hojas de tiempo',
    summarySubTitle: 'Resumen',
    timesheetSubTitle: 'Registro de horas',
    filterByStatus: 'Filtrar por estado',
    allStatuses: 'Todos los estados',
    dateFrom: 'Desde',
    dateTo: 'Hasta',
  },
  timesheet: {
    createTitle: 'Crear registro de horas',
    editTitle: 'Editar hoja de tiempo',
    backToDashboard: 'Volver al panel',
    selectCalendarDate: 'Por favor seleccione una fecha',
    weekSelection: 'Selección de semana',
    weekStartDate: 'Fecha de inicio de semana*',
    weekEndDate: 'Fecha de fin de semana (cubre un período de 7 días)',
    dailyEntries: 'Registros diarios',
    fillOutDays: 'Complete sus días de trabajo o marque los días de descanso.',
    updateDays: 'Actualice sus días de trabajo o marque los días de descanso.',
    daysCompleted: 'Días completados:',
    readyToSubmit: '✓ Listo para enviar',
    dayOff: 'Día libre',
    timeIn: 'Hora de entrada',
    timeOut: 'Hora de salida',
    mileage: 'Millaje',
    tollCharges: 'Cargos de peaje',
    tollCharge: 'Cargo de peaje',
    parkingFee: 'Tarifa de estacionamiento',
    otherCharges: 'Otros cargos',
    comment: 'Comentario',
    commentPlaceholder: 'Notas opcionales...',
    receipts: 'Recibos',
    add: 'Agregar',
    upload: 'Subir',
    uploading: 'Subiendo...',
    noReceipts: 'Sin recibos adjuntos.',
    longDayWarning:
      'Esta entrada registra {{hours}} hrs — por favor verifique sus horarios.',
    futureDay: 'Aún no disponible',
    submitForApproval: 'Enviar para aprobación',
    submittingForApproval: 'Enviando para aprobación...',
    createTimesheet: 'Crear hoja de tiempo',
    saveDraft: 'Guardar borrador',
    confirmSubmitTitle: 'Confirmar el envío de registro de horas',
    confirmSubmitBody:
      'Esta hoja de registro de horas no se puede editar una vez enviada para aprobación.',
    confirmSubmitBtn: 'Confirmar',
    confirmDraftTitle: 'Guardar como borrador',
    confirmDraftBody:
      'Su progreso se guardará y podrá continuar editando esta hoja de registro de horas más tarde.',
    confirmDraftBtn: 'Guardar',
    creating: 'Creando...',
    saveChanges: 'Guardar cambios',
    saving: 'Guardando...',
    weekLabel: 'Semana',
    weekDatesCannotChange: 'Las fechas de la semana no se pueden cambiar',
    editDailylogs: 'Editar hoja de tiempo',
    backToDashboardBtn: 'Volver al panel',
    backToDailylogs: 'Volver a la hoja de horas',
    viewTimesheetTitle: 'Resumen de la hoja de horas',
  },
  dailylogs: {
    tableDate: 'Fecha',
    tableHours: 'Horas',
    tableMileage: 'Millaje',
    tableTollCharges: 'Cargos de peaje',
    tableParkingFees: 'Tarifas de estacionamiento',
    tableOtherCharges: 'Otros cargos',
    tableComments: 'Comentarios',
    tableReceipts: 'Recibos',
    totalHours: 'Total de horas',
    totalMileage: 'Total de millaje',
    totalCharges: 'Total de cargos',
    noDailylogs: 'No se encontraron registros para esta semana',
    denialReasonTitle: 'Esta hoja de tiempo fue rechazada.',
    denialReasonLabel: 'Motivo:',
  },
  admin: {
    title: 'Gestión de usuarios',
    totalUsers: 'Total de usuarios',
    totalSalesReps: 'Total de representantes de ventas',
    totalManagers: 'Total de gerentes',
    totalAdmins: 'Total de administradores',
    activeUsers: 'Usuarios activos',
    filterByRole: 'Filtrar por rol',
    allRoles: 'Todos los roles',
    filterByStatus: 'Filtrar por estado',
    allStatuses: 'Todos los estados',
    users: 'Usuarios',
    managers: 'Gerentes',
    admins: 'Administradores',
    search: 'Buscar',
    searchPlaceholder: 'Buscar por nombre o correo...',
    tableUser: 'Usuario',
    tableEmail: 'Correo',
    tableRole: 'Rol',
    tableManager: 'Gerente',
    tableSalesRep: 'Rep de ventas',
    tableStatus: 'Estado',
    tableActions: 'Acciones',
    active: 'Activo',
    inactive: 'Inactivo',
    noUsers: 'No se encontraron usuarios que coincidan con los filtros',
    editUser: 'Editar usuario',
    removeManager: 'Quitar gerente',
    deactivate: 'Desactivar',
    activate: 'Activar',
    confirmDeactivateTitle: 'Desactivar usuario',
    confirmDeactivateBody:
      '¿Está seguro de que desea desactivar a {{email}}? Ya no podrá iniciar sesión.',
    confirmDeactivateBtn: 'Desactivar',
    confirmActivateTitle: 'Activar usuario',
    confirmActivateBody:
      '¿Está seguro de que desea activar a {{email}}? Recuperará el acceso a la aplicación.',
    confirmActivateBtn: 'Activar',
  },
  userEdit: {
    title: 'Editar usuario',
    backToUsers: 'Volver a usuarios',
    viewTimesheets: 'Ver hojas de tiempo',
    userInfo: 'Información del usuario',
    name: 'Nombre',
    email: 'Correo',
    status: 'Estado',
    currentRole: 'Rol actual',
    currentManager: 'Gerente actual',
    currentSalesRep: 'Rep de ventas actual',
    salesRepAssignment: 'Asignación de rep de ventas',
    assignSalesRep: 'Asignar rep de ventas:',
    noSalesRepAssigned: 'Sin rep de ventas asignado',
    noSalesRepsAvailable:
      'No hay reps de ventas activos disponibles. Asigne el rol de Rep de Ventas a un usuario primero.',
    created: 'Creado',
    lastUpdated: 'Última actualización',
    roleManagement: 'Gestión de roles',
    selectRole: 'Seleccionar rol:',
    managerAssignment: 'Asignación de gerente',
    assignManager: 'Asignar gerente:',
    noManagerAssigned: 'Sin gerente asignado',
    noManagersAvailable:
      'No hay gerentes activos disponibles. Promueva un usuario al rol de Gerente primero.',
    mustAssignManager:
      'Los usuarios deben ser asignados a un gerente para enviar hojas de tiempo',
    rateManagement: 'Gestión de tarifas',
    hourlyRate: 'Tarifa por hora ($)',
    mileageRate: 'Tarifa de millaje ($/mi)',
    effectiveDate: 'Fecha efectiva',
    addRate: 'Agregar tarifa',
    savingRate: 'Guardando...',
    rateHistory: 'Historial de tarifas',
    noRates: 'Sin tarifas establecidas aún.',
    tableEffectiveDate: 'Fecha efectiva',
    tableHourlyRate: 'Tarifa por hora',
    tableMileageRate: 'Tarifa de millaje',
    tableAdded: 'Agregado',
    saveChanges: 'Guardar cambios',
    saving: 'Guardando cambios...',
    cancel: 'Cancelar',
    roleDescUser:
      'Puede crear y enviar hojas de tiempo. Debe estar asignado a un gerente.',
    roleDescManager:
      'Puede aprobar/denegar hojas de tiempo del equipo. No puede enviar sus propias hojas de tiempo.',
    roleDescAdmin:
      'Acceso completo al sistema. Puede gestionar usuarios, roles y ver todos los datos.',
    roleDescSalesRep:
      'Revisa las hojas de tiempo del equipo antes de que lleguen al gerente. No puede enviar sus propias hojas de tiempo.',
    employeeType: 'Tipo de empleado',
    selectEmployeeType: 'Seleccionar tipo de empleado:',
    employeeTypeNotSpecified: 'No especificado',
    employeeTypeEmployee: 'Empleado',
    employeeTypeContractor: 'Contratista 1099',
  },
  manager: {
    title: 'Hojas de tiempo del equipo',
    approved: 'Aprobadas',
    denied: 'Denegadas',
    filterByStatus: 'Filtrar por estado',
    allStatuses: 'Todos los estados',
    pending: 'Pendiente',
    pendingManager: 'En espera de mi revisión',
    pendingSalesRep: 'Pendiente (revisión del rep de ventas)',
    draft: 'Borrador',
    tableEmployee: 'Empleado',
    tableWeek: 'Semana',
    tableHours: 'Horas',
    tableCharges: 'Cargos',
    tableStatus: 'Estado',
    tableSubmitted: 'Enviado',
    tableActions: 'Acciones',
    review: 'Revisar',
    noTimesheets:
      'No se encontraron hojas de tiempo que coincidan con los filtros',
    search: 'Buscar',
    searchPlaceholder: 'Buscar por nombre o correo...',
  },
  salesRep: {
    title: 'Hojas de tiempo del equipo',
    pending: 'Pendientes',
    approved: 'Aprobadas',
    denied: 'Denegadas',
    filterByStatus: 'Filtrar por estado',
    allStatuses: 'Todos los estados',
    pendingSalesRep: 'En espera de mi revisión',
    pendingManager: 'Pendiente (revisión del gerente)',
    draft: 'Borrador',
    tableEmployee: 'Empleado',
    tableWeek: 'Semana',
    tableHours: 'Horas',
    tableCharges: 'Cargos',
    tableStatus: 'Estado',
    tableSubmitted: 'Enviado',
    tableActions: 'Acciones',
    review: 'Revisar',
    noTimesheets:
      'No se encontraron hojas de tiempo que coincidan con los filtros',
    search: 'Buscar',
    searchPlaceholder: 'Buscar por nombre o correo...',
  },
  reviewTimesheet: {
    backToTeam: 'Volver a hojas del equipo',
    title: 'Revisar hoja de tiempo',
    timesheet: 'Resumen de hoja de tiempo',
    weekPeriod: 'Período de la semana',
    submitted: 'Enviado',
    totalHours: 'Total de horas',
    totalCharges: 'Total de cargos',
    salesRepComment: 'Comentario del rep de ventas',
    managerComment: 'Comentario del gerente',
    dailyBreakdown: 'Desglose diario',
    tableDate: 'Fecha',
    tableHours: 'Horas',
    tableMileage: 'Millaje',
    tableTolls: 'Peajes',
    tableParking: 'Estacionamiento',
    tableOther: 'Otros',
    tableComment: 'Comentario',
    tableReceipts: 'Recibos',
    approveTimesheet: 'Aprobar hoja de tiempo',
    approving: 'Aprobando...',
    denyTimesheet: 'Denegar hoja de tiempo',
    cancelDeny: 'Cancelar denegación',
    denyTitle: 'Denegar hoja de tiempo',
    employeeNotified:
      'El empleado será notificado y podrá revisar su hoja de tiempo.',
    reasonForDenial: 'Motivo de la denegación*',
    reasonPlaceholder:
      'Por favor explique por qué se está denegando esta hoja de tiempo para que el empleado pueda hacer correcciones...',
    confirmDenial: 'Confirmar denegación',
    denying: 'Denegando hoja de tiempo...',
    cancel: 'Cancelar',
    confirmApproveTitle: 'Aprobar hoja de tiempo',
    confirmApproveBody:
      '¿Está seguro de que desea aprobar la hoja de tiempo de {{name}}?',
    confirmApproveBtn: 'Aprobar',
    confirmDenyTitle: 'Denegar hoja de tiempo',
    confirmDenyBody:
      '¿Está seguro de que desea denegar esta hoja de tiempo? El empleado será notificado.',
    confirmDenyBtn: 'Denegar',
  },
  adminTimesheetDetail: {
    backToTimesheets: 'Volver a hojas de tiempo',
    title: 'Detalle de hoja de tiempo',
    summary: 'Resumen',
    weekPeriod: 'Período de la semana',
    submitted: 'Enviado',
    totalHours: 'Total de horas',
    totalCharges: 'Total de cargos',
    managerComment: 'Comentario del gerente',
    dailyBreakdown: 'Desglose diario',
    tableDate: 'Fecha',
    tableHours: 'Horas',
    tableMileage: 'Millaje',
    tableTolls: 'Peajes',
    tableParking: 'Estacionamiento',
    tableOther: 'Otros',
    tableComment: 'Comentario',
    tableReceipts: 'Recibos',
  },
  adminUserTimesheets: {
    backToUser: 'Volver al usuario',
    title: 'Hojas de tiempo',
    tableWeek: 'Semana',
    tableStatus: 'Estado',
    tableTotalHours: 'Total de horas',
    tableTotalCharges: 'Total de cargos',
    tableSubmitted: 'Enviado',
    noTimesheets: 'No se encontraron hojas de tiempo para este usuario',
    view: 'Ver',
  },
  days: {
    sunday: 'Domingo',
    monday: 'Lunes',
    tuesday: 'Martes',
    wednesday: 'Miércoles',
    thursday: 'Jueves',
    friday: 'Viernes',
    saturday: 'Sábado',
  },
  months: {
    january: 'Enero',
    february: 'Febrero',
    march: 'Marzo',
    april: 'Abril',
    may: 'Mayo',
    june: 'Junio',
    july: 'Julio',
    august: 'Agosto',
    september: 'Septiembre',
    october: 'Octubre',
    november: 'Noviembre',
    december: 'Diciembre',
  },
  status: {
    pending: 'Pendiente',
    pendingsalesrep: 'Pendiente (Rep de Ventas)',
    pendingmanager: 'Pendiente (Gerente)',
    approved: 'Aprobada',
    denied: 'Denegada',
    draft: 'Borrador',
  },
  toast: {
    loggedOut: '¡Ha cerrado sesión exitosamente!',
    receiptUploadFailed: 'Algunos recibos no se pudieron subir',
    onlyDraftEditable: 'Solo se pueden editar hojas de tiempo en borrador',
    failedLoadTimesheet: 'Error al cargar la hoja de tiempo',
    receiptUploaded: '¡Recibo subido exitosamente!',
    failedUploadReceipt: 'Error al subir el recibo',
    receiptDeleted: 'Recibo eliminado.',
    failedDeleteReceipt: 'Error al eliminar el recibo',
    failedDownloadReceipt: 'Error al descargar el recibo',
    timesheetUpdated: '¡Hoja de tiempo actualizada exitosamente!',
    invalidFileType: 'Solo se aceptan archivos JPG, PNG y PDF.',
    fileTooLarge: 'El archivo debe ser menor a 10 MB.',
    atLeastOneDayRequired:
      'Por favor complete al menos un día antes de guardar.',
    dayErrorsOnSubmit:
      '{{count}} día(s) tienen errores — por favor revise las entradas resaltadas.',
    failedCreateTimesheet:
      'No se pudo crear la hoja de tiempo — por favor intente de nuevo.',
    failedSaveDailyLogs:
      'El borrador fue creado pero las entradas diarias no se guardaron — edite el borrador desde su panel.',
    failedUpdateTimesheet:
      'No se pudieron guardar los cambios — por favor intente de nuevo.',
    onlyPendingApprovable: 'Solo se pueden aprobar hojas de tiempo pendientes',
    timesheetApproved: '¡Hoja de tiempo aprobada exitosamente!',
    failedApproveTimesheet: 'Error al aprobar la hoja de tiempo',
    onlyPendingDeniable: 'Solo se pueden denegar hojas de tiempo pendientes',
    timesheetDenied: 'Hoja de tiempo denegada',
    failedDenyTimesheet: 'Error al denegar la hoja de tiempo',
    failedLoadUser: 'Error al cargar el usuario',
    noChanges: 'No hay cambios para guardar',
    roleUpdated: '¡Rol actualizado exitosamente!',
    failedUpdateRole: 'Error al actualizar el rol',
    managerRemoved: '¡Gerente eliminado exitosamente!',
    failedRemoveManager: 'Error al eliminar al gerente',
    managerAssigned: '¡Gerente asignado exitosamente!',
    failedAssignManager: 'Error al asignar el gerente',
    employeeTypeUpdated: '¡Tipo de empleado actualizado exitosamente!',
    failedUpdateEmployeeType: 'Error al actualizar el tipo de empleado',
    rateSaved: '¡Tarifa guardada exitosamente!',
    failedSaveRate: 'Error al guardar la tarifa',
    failedLoadTimesheets: 'Error al cargar las hojas de tiempo',
    salesRepAssigned: '¡Rep de ventas asignado exitosamente!',
    failedAssignSalesRep: 'Error al asignar el rep de ventas',
    salesRepRemoved: '¡Rep de ventas eliminado exitosamente!',
    failedRemoveSalesRep: 'Error al eliminar el rep de ventas',
  },
  payrollBanner: {
    submittedAnd: 'Las hojas de tiempo enviadas y',
    approved: 'aprobadas',
    for: 'para',
    areScheduledPaymentOn: 'están programadas para pago el',
    ifThisTimesheetIs: 'Si esta hoja de tiempo es',
    before: 'antes del',
    itIsScheduledPaymentOn: ', está programada para pago el',
  },
};
