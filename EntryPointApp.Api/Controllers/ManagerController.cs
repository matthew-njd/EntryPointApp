    ...
    public class ManagerController(IManagerService managerService, ILogger<ManagerController> logger) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTimesheetStatusHistory([FromRoute] int id)
        {
            try
            {
                ...
                var result = await _managerService.GetTimesheetStatusHistoryAsync(id, managerId);
                ...
                _logger.LogInformation("Manager {ManagerId} retrieved status history for timesheet {TimesheetId}", managerId, id);
